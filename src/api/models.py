...
class User(db.Model):
    ...
    password: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)  # Nullable para guest checkout
    ...
    verification_token: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, unique=True)
    verification_token_expires: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    
    # NUEVO: Reset de contraseña
    password_reset_token: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, unique=True)
    ...
    # Relaciones
    bookings: Mapped[List["Booking"]] = relationship(back_populates='user', lazy='dynamic')
...
class Experience(db.Model):
    ...
    schedules: Mapped[List["ExperienceSchedule"]] = relationship(back_populates='experience', cascade='all, delete-orphan')
    bookings: Mapped[List["Booking"]] = relationship(back_populates='experience')
...
class ExperienceSchedule(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    experience_id: Mapped[int] = mapped_column(Integer, db.ForeignKey('experiences.id'), nullable=False)
...
class Package(db.Model):
    ...
    image_url: Mapped[Optional[str]] = mapped_column(String(500), nullable=True)
    is_active: Mapped[bool] = mapped_column(Boolean(), default=True)
    room_id: Mapped[Optional[int]] = mapped_column(Integer, db.ForeignKey('rooms.id'), nullable=True)
    experience_id: Mapped[Optional[int]] = mapped_column(Integer, db.ForeignKey('experiences.id'), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=datetime.utcnow)
    
    room: Mapped[Optional["Room"]] = relationship()
    experience: Mapped[Optional["Experience"]] = relationship()
    included_extras: Mapped[List["PackageExtra"]] = relationship(back_populates='package', cascade='all, delete-orphan')
...
class PackageExtra(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    package_id: Mapped[int] = mapped_column(Integer, db.ForeignKey('packages.id'), nullable=False)
    extra_id: Mapped[int] = mapped_column(Integer, db.ForeignKey('extras.id'), nullable=False)
...
class Booking(db.Model):
    ...
    experience_id: Mapped[Optional[int]] = mapped_column(Integer, db.ForeignKey('experiences.id'), nullable=True)
    package_id: Mapped[Optional[int]] = mapped_column(Integer, db.ForeignKey('packages.id'), nullable=True)
    
    experience_date: Mapped[Optional[datetime]] = mapped_column(Date, nullable=True)
    experience_time: Mapped[Optional[time]] = mapped_column(Time, nullable=True)
    ...
    status: Mapped[BookingStatus] = mapped_column(SQLEnum(BookingStatus), default=BookingStatus.CART)
    ...
    cart_expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime, nullable=True)
    
    # Relaciones
    user: Mapped["User"] = relationship(back_populates='bookings')
    experience: Mapped[Optional["Experience"]] = relationship(back_populates='bookings')
    package: Mapped[Optional["Package"]] = relationship(back_populates='bookings')
    rooms: Mapped[List["BookingRoom"]] = relationship(back_populates='booking', cascade='all, delete-orphan')
    extras: Mapped[List["BookingExtra"]] = relationship(back_populates='booking', cascade='all, delete-orphan')
    email_logs: Mapped[List["EmailLog"]] = relationship(back_populates='booking', cascade='all, delete-orphan')
...
class BookingRoom(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    booking_id: Mapped[int] = mapped_column(Integer, db.ForeignKey('bookings.id'), nullable=False)
    room_id: Mapped[int] = mapped_column(Integer, db.ForeignKey('rooms.id'), nullable=False)
...
class EmailLog(db.Model):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    booking_id: Mapped[int] = mapped_column(Integer, db.ForeignKey('bookings.id'), nullable=False)
    email_type: Mapped[str] = mapped_column(String(50), nullable=False)  # 'booking_confirmation', 'payment_receipt', etc.
...
class ExperienceAvailability(db.Model):
    ...
    # Item Type (experience or room)
    item_type = db.Column(db.String(50), nullable=False)  # 'experience' or 'room'
    
    # Reference to the actual item
    experience_id = db.Column(db.Integer, db.ForeignKey('experiences.id'), nullable=True)
    room_id = db.Column(db.Integer, db.ForeignKey('rooms.id'), nullable=True)
    
    # Item Details (cached for historical record)
    name = db.Column(db.String(200), nullable=False)
    image_url = db.Column(db.String(500))
    
    # Experience-specific fields
    date = db.Column(db.Date, nullable=True)  # For experiences
    guests = db.Column(db.Integer)
    
    # Room-specific fields
    check_in = db.Column(db.Date, nullable=True)  # For rooms
    check_out = db.Column(db.Date, nullable=True)  # For rooms
